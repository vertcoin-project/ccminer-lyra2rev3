...
__global__ __launch_bounds__(TPB35, 1)
void lyra2v3_gpu_hash_32_v3_targ(uint32_t threads, uint32_t startNounce, uint2 *outputHash, uint32_t *resultnonces)
{
	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	...
	if (thread < threads)
	{
		...
		if  (((uint64_t) ((uint2*)state[0])) <= pTarget[3])
		{
...
__global__ void lyra2v3_gpu_hash_32_v3_targ(uint32_t threads, uint32_t startNounce, uint2 *outputHash, uint32_t *resultnonces) {}
#endif
